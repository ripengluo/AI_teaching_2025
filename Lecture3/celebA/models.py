...
class VAE(nn.Module):
    ...
    def __init__(self, latent_dim=10, device="cuda"):
        ...
        self.mean_layer = nn.Linear(256 * 8 * 8, latent_dim)
        self.logvar_layer = nn.Linear(256 * 8 * 8, latent_dim)

        # decoder
        self.predecode = nn.Linear(latent_dim, 256 * 8 * 8)
...
class ParallelNetworks(nn.Module):
    def __init__(self, order, latent_dim):
        super().__init__()
        self.order = order
    ...
    @torch.compile
    def forward(self, u):
        u = u.unsqueeze(0).expand(self.order, -1, -1)
        return torch.stack([net(u[i]) for i, net in enumerate(self.nets)])
