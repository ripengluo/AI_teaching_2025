...
def train_mivae(model, x_, optimizer, scheduler, train_loader, log, order, epochs, batch_size, device, x_flat_dim):
    model.train()
    x_ = torch.cat([x_]*batch_size)
    for epoch in range(epochs):
        overall_loss = 0
        for batch_idx, (x, _) in enumerate(train_loader):
            x = x.to(device)
            ...
            x_hat, mean_post, logvar_post, mean_prior, logvar_prior = model(x, x_)
            loss = loss_mivae(x, x_hat, mean_post, logvar_post, mean_prior, logvar_prior, order, epoch)
        ...
        if epoch == 0 or loss_min > overall_loss:
            loss_min = overall_loss
            if epoch > int(0.5 * epochs):
                torch.save(model.state_dict(), "mivae.pth.%d"%order)
                print("model saved!")
